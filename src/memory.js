...
function saveMemory(facts) {
    const safeStorage = getSafeStorage();
    if (facts.length && !safeStorage?.isEncryptionAvailable())
        throw new Error('Secure memory storage is unavailable. Unlock your operating system keychain and retry.');
    const encrypted = facts.map(f => ({ _enc: safeStorage.encryptString(JSON.stringify(f)).toString('base64') }));
    if (!writeJsonFile(getMemoryPath(), { [ENCRYPTION_MARKER]: MARKER_VALUE, facts: encrypted })) throw new Error('Could not save memory');
    return true;
}
