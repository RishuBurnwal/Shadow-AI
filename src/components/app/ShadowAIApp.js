...
export class ShadowAIApp extends LitElement {
    static styles = css`
        * {
        ...
        @media (max-width: 1400px) {
            .top-drag-bar .secondary-header-control {
                display: none;
            }
            .top-drag-bar .header-more {
                display: block;
            }
        }
        ...
        .persistent-opacity .header-opacity {
            width: 100%;
            margin: 0;
            height: 16px;
        }
        .top-drag-bar .provider-select-wrap {
            min-width: 70px;
            flex: 1 1 110px;
        }
        .top-drag-bar .audio-mode-select {
            min-width: 70px;
            flex: 1 1 95px;
        }
        @media (max-width: 900px) {
    ...
    async _loadFromStorage() {
        try {
            ...
            this.screenAnalysisMode = window.ShadowContextPolicy.resolve(prefs).screen;
            this.selectedImageQuality = prefs.selectedImageQuality || 'medium';
            this.layoutMode = config.layout || 'normal';
            this.backgroundTransparency = prefs.backgroundTransparency ?? 0.8;
            document.documentElement.style.setProperty('--response-font-size', `${prefs.fontSize || 20}px`);
            ...
            this.focusLock = Boolean(prefs.focusLock);
    ...
    connectedCallback() {
        ...
        this._modelsChangedHandler = () => this.loadProviderStatus();
        window.addEventListener('provider-models-changed', this._modelsChangedHandler);
        this.loadProviderStatus();
    ...
    disconnectedCallback() {
        ...
        window.removeEventListener('provider-models-changed', this._modelsChangedHandler);
    }
    ...
    providerOptionLabel(provider) {
        ...
        return `${label} (${status?.keyCount || 0} keys) — ${status?.message || 'Checking'}`;
    }
    ...
    async handleStart() {
        ...
        if (providerMode === 'local') {
            ...
            const initialized = await shadowAI.initializeGemini(this.selectedProfile, this.selectedLanguage);
            if (!initialized) {
                this.setStatus('Voice connection failed. Check transcription credentials and try again.');
                return;
            }
        }
    ...
    async handleScreenAnalysisModeChange(mode) {
        this.screenAnalysisMode = ['off', 'manual', 'automatic'].includes(mode) ? mode : 'manual';
        const prefs = await shadowAI.storage.getPreferences();
        const active = prefs.activeContextProfiles?.[prefs.selectedProfile];
        if (active)
            await shadowAI.storage.updatePreference(
                'contextProfiles',
                prefs.contextProfiles.map(p => (p.id === active ? { ...p, rules: { ...p.rules, screen: this.screenAnalysisMode } } : p))
            );
        else
            await shadowAI.storage.updatePreference('contextRules', {
                ...prefs.contextRules,
                [prefs.selectedProfile]: { ...window.ShadowContextPolicy.resolve(prefs), screen: this.screenAnalysisMode },
            });
        window.dispatchEvent(new CustomEvent('context-settings-changed'));
        this.requestUpdate();
    }
        ...
        return html`
            <div class="app-shell">
                ...
                <div class="top-drag-bar">
                    ...
                    <div class="drag-region" title="Drag window" aria-label="Drag window">&#x283f;</div>
                    ...
                    <select
                        ...
                        ?disabled=${!modelProvider || !modelStatus?.configured || !modelStatus?.models?.length}
                    ...
                    <label class="header-control persistent-opacity" title="Uses the Background Transparency setting">
                        <span>Screen ${Math.round(this.backgroundTransparency * 100)}%</span>
                        <input
                    ...
                    <label class="header-control persistent-opacity" title="Controls only AI response text opacity">
                    ...
                    <details class="header-more">
                        ...
                        <div class="header-more-panel">
                            <button
                                aria-pressed=${this.focusLock}
                                @click=${async () => {
                                    const result = await window.electronAPI.ipcRenderer.invoke('set-focus-lock', !this.focusLock);
                                    if (result.success) this.focusLock = result.enabled;
                                }}
                            >
                                Focus lock: ${this.focusLock ? 'ON · click to enable typing' : 'OFF · click for mouse-only overlay'}
                            </button>
                            <span style="font-size:11px"
                                >When locked, keyboard input stays in the other app. This does not guarantee undetectability.</span
                            >
                            <select
