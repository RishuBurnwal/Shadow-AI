...
import './ModelSettings.js';
import './ContextSettings.js';
...
export class CustomizeView extends LitElement {
    ...
    constructor() {
        ...
        this.vadSilenceMs = 1200;
        this.responseDelayMs = 750;
    ...
    async _loadFromStorage() {
        try {
            ...
            this.vadSilenceMs = prefs.vadSilenceMs ?? 1200;
            this.responseDelayMs = prefs.responseDelayMs ?? 750;
    ...
    getProfiles() {
        return [
            ...
            { value: 'quiz', name: 'Quiz Assistant' },
        ];
    }
    ...
    getDefaultKeybinds() {
        const isMac = shadowAI.isMacOS || navigator.platform.includes('Mac');
        return {
    ...
    async restoreAllSettings() {
        ...
        this.requestUpdate();
        try {
            // Reset general controls without deleting documents, profiles, or models.
            const defaults = {
            ...
            for (const [key, value] of Object.entries(defaults)) {
                await shadowAI.storage.updatePreference(key, value);
            }
            ...
            this.automaticResponse = defaults.automaticResponse;
            this.interviewCaptureMode = defaults.interviewCaptureMode;
            const app = document.querySelector('shadow-ai-app');
            if (app) {
                await app.handleBackgroundTransparencyChange(defaults.backgroundTransparency);
                await app.handleResponseTextOpacityChange(defaults.responseTextOpacity);
                app.responseTextColor = defaults.responseTextColor;
                await app.handleAutomaticResponseChange(defaults.automaticResponse);
                await app.handleAudioModeChange(defaults.audioMode);
                await app.handleInterviewCaptureModeChange(defaults.interviewCaptureMode);
                await app.handleScreenshotIntervalChange(defaults.selectedScreenshotInterval);
                app.focusLock = false;
            }
            if (window.electronAPI.platform === 'win32') await window.electronAPI.ipcRenderer.invoke('set-focus-lock', false);
            ...
            this.updateBackgroundAppearance();
            this.updateFontSize();
            await shadowAI.theme.save(defaults.theme);

            this.clearStatusMessage =
                'General settings restored. Documents, context profiles, and model selections retained. Restart the session for speech-provider changes.';
    ...
    async clearLocalData() {
        ...
        this.requestUpdate();
        try {
            await shadowAI.storage.clearAll();
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group slider-wrap">
                        <div class="slider-header">
                            <label class="form-label">Question finished after continuous silence</label>
                        ...
                        />
                        <div class="form-help">
                            A shorter pause stays in the same question. Listening continues for the next question; use Pause to stop capture. Hosted
                            batch/local audio use this value. Gemini Live supports up to 2 seconds per server turn and needs a session restart for
                            changes.
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Extra wait before answering (seconds)</label>
                        <input
                            class="control"
                            type="number"
                            min="0"
                        ...
                        />
                        <div class="form-help">
                            Used only in Automatic mode after silence and transcription complete. New speech resets this timer and joins the same
                            question. In Manual mode there is no timer; generation waits for OK. Current: ${this.responseDelayMs} ms.
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">When to answer</label>
                        <select
                            class="control"
                            aria-label="When to answer"
                            .value=${this.automaticResponse ? 'automatic' : 'manual'}
                            @change=${async e => {
                                this.automaticResponse = e.target.value === 'automatic';
                                const app = document.querySelector('shadow-ai-app');
                                if (app) await app.handleAutomaticResponseChange(this.automaticResponse);
                                else await shadowAI.storage.updatePreference('automaticResponse', this.automaticResponse);
                            }}
                        >
                            <option value="automatic">Automatic · silence + delay</option>
                            <option value="manual">Manual · wait until I click OK</option>
                        </select>
                        <div class="form-help">For long or unpredictable pauses, Manual lets you decide when the question is complete.</div>
                    </div>
                </div>
            </section>
        `;
    ...
    renderPrivacySection() {
        return html`
            <div class="form-group">
                <label
                    ><input type="checkbox" .checked=${this.googleSearchEnabled} @change=${this.handleGoogleSearchChange} /> Google Search for Gemini
                    answers</label
                >
                <div class="form-description">Allows Gemini to search the web when needed. Search can add provider usage charges.</div>
            </div>
            ...
            <section class="surface danger-surface">
                ...
                <div style="display:flex;gap:var(--space-sm);flex-wrap:wrap;">
                    <button class="danger-button" @click=${this.restoreAllSettings} ?disabled=${this.isRestoring}>
                        ${this.isRestoring ? 'Restoring...' : 'Restore general settings'}
                    </button>
        ...
        return html`
            <div class="unified-page">
                <div class="unified-wrap">
                    ...
                    <context-settings></context-settings><model-settings></model-settings> ${this.renderPrivacySection()}
                </div>
            </div>
        `;
    }
