...
export class MainView extends LitElement {
    ...
    async _loadFromStorage() {
        try {
            ...
            if (storedMode === 'cloud') {
                await shadowAI.storage.updatePreference('providerMode', this._mode);
            }
    ...
    async _saveGeminiKey(val) {
        this._geminiKey = val;
        this._keyError = false;
        await shadowAI.setProviderApiKey('gemini', val);
        await this._refreshProviderConfiguration();
        this.requestUpdate();
    }

    async _saveGroqKey(val) {
        this._groqKey = val;
        await shadowAI.setProviderApiKey('groq', val);
        await this._refreshProviderConfiguration();
        this.requestUpdate();
    }
    ...
    async _saveOpenaiKey(val) {
        ...
        try {
            ...
            await shadowAI.storage.setCredentials({ ...creds, openaiKey: val });
        } catch (e) {}
        this.requestUpdate();
    }

    async _saveAdditionalApiKey(provider, credential, value) {
        this._apiKeys = { ...this._apiKeys, [credential]: value };
        await shadowAI.setProviderApiKey(provider, value);
        await this._refreshProviderConfiguration();
        this.requestUpdate();
    }

    async _refreshProviderConfiguration() {
        const activeElement = this.shadowRoot?.activeElement;
        if (activeElement?.matches?.('input[type="password"]')) return;
        const status = await shadowAI.getProviderStatus().catch(() => null);
        if (!status) return;
        const credentials = await shadowAI.storage.getCredentials().catch(() => ({}));
        this._providerStatus = status;
        this._geminiKey = credentials.apiKey || '';
        this._groqKey = credentials.groqApiKey || '';
        this._apiKeys = {
    ...
    async _saveOllamaHost(val) {
        ...
        await shadowAI.storage.updatePreference('ollamaHost', val);
        this.requestUpdate();
    }

    async _saveOllamaModel(val) {
        this._ollamaModel = val;
        await shadowAI.storage.updatePreference('ollamaModel', val);
        this.requestUpdate();
    }

    async _saveWhisperModel(val) {
        this._whisperModel = val;
        await shadowAI.storage.updatePreference('whisperModel', val);
        this.requestUpdate();
    }
    ...
    _handleStart() {
        ...
        if (this._mode === 'byok') {
            if (!this._providerStatus.providerIds?.some(id => this._providerStatus[id])) {
                this._keyError = true;
                this.requestUpdate();
                return;
            }
    ...
    _renderByokMode() {
        ...
        return html`
            <div class="form-group">
                ...
                <div class="form-hint">
                    Your context profile controls what is sent. Voice uses the selected transcription provider. Enter multiple API keys separated by
                    commas; duplicate keys count once.
                </div>
                <input
                    type="password"
                    placeholder="Optional · keys separated by commas"
                    .value=${this._geminiKey}
                    @change=${e => this._saveGeminiKey(e.target.value)}
                    class=${this._keyError ? 'error' : ''}
                />
                <div class="form-hint">
                    <span class="link" @click=${() => this.onExternalLink('https://aistudio.google.com/apikey')}>Get Gemini key</span>
                    <span> · ${this._providerStatus.providers?.gemini?.keyCount || 0} keys configured</span>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <input
                    ...
                    .value=${this._groqKey}
                    @change=${e => this._saveGroqKey(e.target.value)}
                />
                <div class="form-hint">
                    <span class="link" @click=${() => this.onExternalLink('https://console.groq.com/keys')}>Get Groq key</span>
                    <span> · ${this._providerStatus.providers?.groq?.keyCount || 0} keys configured</span>
                    ${
                        Object.entries(this._providerStatus).some(([name, enabled]) => name !== 'selected' && enabled)
                            ? html`<span> · Auto fallback providers loaded from .env</span>`
                            : ''
                    }
                </div>
            </div>
                              ...
                              provider =>
                                  html`<div class="form-group">
                                      ...
                                      <input
                                          type="password"
                                          autocomplete="off"
                                          placeholder=${this._providerStatus[provider.status] ? 'Loaded from .env' : 'Keys separated by commas'}
                                      ...
                                      <div class="form-hint">
                                          ...
                                          <span> · ${this._providerStatus.providers?.[provider.status]?.keyCount || 0} keys configured</span>
                                      </div>
                                  </div>`
                          )}
                      </div>`
            ...
            <div class="form-wrapper">
                ${
                    this._mode === 'local'
                        ? html`
                              <div class="title-row">
                                  <div class="page-title">Shadow AI <span class="mode-suffix">Local AI</span></div>
                                  <button
                                      class="help-btn"
                                      @click=${() => {
                          ...
                          `
                        : html` <div class="page-title">${html`Shadow AI <span class="mode-suffix">BYOK</span>`}</div> `
                }
