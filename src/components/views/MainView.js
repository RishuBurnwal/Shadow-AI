...
export class MainView extends LitElement {
    ...
    constructor() {
        ...
        this._animId = null;
        this._providerRefreshTimer = null;
        this._time = 0;
        this._mouseX = -1;
        this._mouseY = -1;
    ...
    connectedCallback() {
        ...
        this._providerRefreshTimer = setInterval(() => this._refreshProviderConfiguration(), 2000);
    }

    disconnectedCallback() {
        super.disconnectedCallback();
        document.removeEventListener('keydown', this.boundKeydownHandler);
        clearInterval(this._providerRefreshTimer);
    ...
    async _saveGeminiKey(val) {
        this._geminiKey = val;
        this._keyError = false;
        const result = await shadowAI.setProviderApiKey('gemini', val);
        if (!result?.success) await this._refreshProviderConfiguration();
        this.requestUpdate();
    }

    async _saveGroqKey(val) {
        this._groqKey = val;
        const result = await shadowAI.setProviderApiKey('groq', val);
        if (!result?.success) await this._refreshProviderConfiguration();
        this.requestUpdate();
    }
    ...
    async _saveAdditionalApiKey(provider, credential, value) {
        ...
        const result = await shadowAI.setProviderApiKey(provider, value);
        if (!result?.success) await this._refreshProviderConfiguration();
        this._providerStatus = await shadowAI.getProviderStatus().catch(() => this._providerStatus);
        this.requestUpdate();
    }
    ...
    _handleStart() {
        ...
        if (this._mode === 'byok') {
            if (!this._geminiKey.trim() && !this._providerStatus.gemini) {
                this._keyError = true;
                this.requestUpdate();
                return;
            }
        ...
        return html`
            <div class="form-group">
                ...
                <div class="form-hint">
                    ...
                    ${this._providerStatus.gemini ? html`<span> · Loaded securely from .env</span>` : ''}
                </div>
            </div>

            <div class="form-group">
                <label class="form-label">Groq API Key</label>
                <input type="password" placeholder="Optional" .value=${this._groqKey} @change=${e => this._saveGroqKey(e.target.value)} />
                <div class="form-hint">
