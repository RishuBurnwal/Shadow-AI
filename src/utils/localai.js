...
const { createOpenAiUserMessage } = require('./multimodal');
...
let ollamaHostUrl = null;
let localAnswerAbort = null;
...
let pendingFinalTranscriptions = 0;
let speechBuffers = [];
...
function setVadSilenceMs(value) {
    const milliseconds = require('./turnDebouncer').normalizeSilenceMs(value);
...
async function processVAD(pcm16kBuffer) {
    ...
    if (isVoice) {
        ...
        if (isSpeaking && silenceFrameCount >= vadConfig.silenceFramesRequired) {
            ...
            speechBuffers = [];
            pendingFinalTranscriptions++;
            try {
                await handleSpeechEnd(audioData);
            } finally {
                pendingFinalTranscriptions--;
                answerDebouncer.hold(isSpeaking || pendingFinalTranscriptions > 0);
            }
            return;
        }
    }
    ...
    answerDebouncer.hold(isSpeaking || speechFrameCount > 0 || pendingFinalTranscriptions > 0);
    if (isSpeaking) {
...
async function loadNemotronServer() {
    ...
    const ready = await fetch(new URL('/ready', endpoint), { headers, signal: AbortSignal.timeout(3000) });
...
async function handleSpeechEnd(audioData) {
    ...
    if (transcription && transcription.trim().length > 0) {
        ...
        getGemini().publishFinalQuestion(transcription.trim());
    }
...
async function sendToOllama(transcription, screenshot = getGemini().getRecentScreenshot()) {
    const prefs = require('../storage').getPreferences();
    const policy = require('./contextPolicy').resolve(prefs);
    currentSystemPrompt = getSystemPrompt(prefs.selectedProfile, '', false);
    if (policy.screen === 'off') screenshot = null;
    const signature = JSON.stringify(policy);
    if (contextSignature !== signature) localConversationHistory = [];
    contextSignature = signature;
    if (!ollamaClient || !ollamaModel) return { success: false, error: 'No active local session' };
    if (localAnswerAbort) return { success: false, error: 'A local answer is already running' };
    const controller = new AbortController();
    localAnswerAbort = controller;
    try {
        let models = [ollamaModel];
        if (screenshot) {
            const installed = await fetch(`${ollamaHostUrl}/api/tags`, { signal: AbortSignal.any([controller.signal, AbortSignal.timeout(3000)]) });
            if (!installed.ok) throw new Error(`Ollama model discovery failed (${installed.status})`);
            const names = [...new Set([ollamaModel, ...((await installed.json()).models || []).map(m => m.name)])];
            models = [];
            for (const model of names) {
                const response = await fetch(`${ollamaHostUrl}/api/show`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ model }),
                    signal: AbortSignal.any([controller.signal, AbortSignal.timeout(2000)]),
                });
                if (response.ok && (await response.json()).capabilities?.includes('vision')) models.push(model);
            }
            if (!models.length) throw new Error('No installed Ollama model supports vision. Install a vision model in Ollama.');
        }
        let first = true;
        const result = await streamWithFallback({
            providers: models.map(model => ({
                ...
                model,
                apiKey: 'ollama',
                baseUrl: `${ollamaHostUrl}/v1`,
                vision: Boolean(screenshot),
            })),
            messages: [
                { role: 'system', content: currentSystemPrompt || 'You are a helpful assistant.' },
                ...(policy.history ? localConversationHistory.slice(-2) : []),
            ...
            totalTimeoutMs: screenshot ? 15000 : 60000,
            onToken: (token, text) => {
                sendToRenderer(first ? 'new-response' : 'update-response', text);
                first = false;
            },
        });
        localConversationHistory.push({ role: 'user', content: transcription.trim() }, { role: 'assistant', content: result.text });
        localConversationHistory = localConversationHistory.slice(-20);
        saveConversationTurn(transcription, result.text);
        sendToRenderer('update-status', result.partial ? `Answer interrupted: ${result.warning}` : 'Listening...');
        return { success: true, ...result };
    } catch (error) {
        sendToRenderer('update-status', `Ollama: ${error.message}`);
        return { success: false, error: error.message };
    } finally {
        if (localAnswerAbort === controller) localAnswerAbort = null;
    }
}
...
async function initializeLocalSession(ollamaHost, model, whisperModel, profile, customPrompt) {
    ...
    try {
        ...
        const endpoint = new URL(ollamaHost);
        if (!['http:', 'https:'].includes(endpoint.protocol)) throw new Error('Ollama host must use HTTP or HTTPS');
        ollamaHostUrl = endpoint.href.replace(/\/$/, '');
        ollamaClient = new Ollama({ host: ollamaHostUrl });
        ollamaModel = model;
...
async function sendLocalText(text) {
    return sendToOllama(text);
}

async function sendLocalImage(base64Data, prompt) {
    return sendToOllama(prompt || 'Analyze this screen concisely.', base64Data);
}
