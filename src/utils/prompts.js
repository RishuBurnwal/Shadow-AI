...
function buildProfileSummary() {
    try {
        const p = getProfile();
        if (!p || (!p.resumeText && !p.name && !p.targetRole && !p.experienceSummary && p.keySkills.length === 0 && p.pastProjects.length === 0)) {
...
function buildSystemPrompt(promptParts, customPrompt = '', googleSearchEnabled = true, profile = 'interview', enabledSkills = null) {
    ...
    const policy = require('./contextPolicy').resolve(preferences, profile);
    ...
    const profileSummary = policy.resume ? buildProfileSummary() : '';
    if (profileSummary) {
        sections.push('\n\n', profileSummary);
    }

    // Inject relevant memory facts (keyword-ranked + recency-scored)
    const memorySummary = policy.memory ? buildMemorySummary(customPrompt) : '';
    if (memorySummary) {
        sections.push('\n\n', memorySummary);
    }

    // Inject skill prompt fragments (respecting enabled/disabled toggles)
    const skillFragments = policy.skills ? getSkillPromptFragments(profile, policy.skillIds || enabledSkills) : [];
    for (const fragment of skillFragments) {
        sections.push('\n', fragment.slice(0, 1800));
    }

    const context = require('./contextPolicy').userContext(preferences, policy);
    // Do not use the legacy combined string: it can contain a JD even when JD is disabled.
    sections.push('\n\nUser-provided context\n-----\n', context, '\n-----\n\n', promptParts.outputInstructions);

    return sections.join('').slice(0, 22000);
}

function getSystemPrompt(profile, customPrompt = '', googleSearchEnabled = true, enabledSkills = null) {
    const promptParts = profilePrompts[profile === 'quiz' ? 'exam' : profile] || profilePrompts.interview;
