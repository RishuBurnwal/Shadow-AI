...
function syncProviderEnvironment() {
    ...
    if (!env) return require('../storage').getCredentials();

    const { defaultCredentials } = require('./providers.config');
    ...
    for (const { envKey, credential, modelEnv } of Object.values(PROVIDER_KEYS)) {
        const value =
            ...
            Object.entries(env).find(
                ([key, value]) => key.startsWith(`${envKey}_`) && /^\d+$/.test(key.slice(envKey.length + 1)) && value && !value.startsWith('#')
            )?.[1] ||
            '';
        credentials[credential] = require('./apiKeys').providerApiKeys(env, envKey).join(',');
        ...
        const visionEnv = modelEnv.replace(/_MODEL$/, '_VISION_MODEL');
        if (env[visionEnv]) process.env[visionEnv] = env[visionEnv];
        else delete process.env[visionEnv];
        if (env[modelEnv]) process.env[modelEnv] = env[modelEnv];
        else delete process.env[modelEnv];
    }
    return credentials;
}
...
function setProviderKey(provider, value) {
    ...
    // The UI edits the complete key list, including any legacy numbered entries.
    const consolidated = current
        .split(/\r?\n/)
        .filter(line => !new RegExp(`^\\s*(?:export\\s+)?${definition.envKey}_\\d+\\s*=`).test(line))
        .join('\n');
    const updated = replaceEnvValue(consolidated, definition.envKey, require('./apiKeys').splitApiKeys(safeValue).join(','));
