...
const DEFAULT_PREFERENCES = {
    ...
    activeLocalSttEngine: 'Not loaded',
    vadSilenceMs: 1200,
    responseDelayMs: 750,
    focusLock: false,
...
function runPreferencesMigrations(data) {
    ...
    return normalizeProviderPreferences(PREFERENCES_MIGRATIONS[0](data || {}));
}

function normalizeProviderPreferences(preferences) {
    const ids = new Set(require('./utils/providers.config').PROVIDERS.map(provider => provider.id));
    const filter = value => Object.fromEntries(Object.entries(value || {}).filter(([id]) => ids.has(id)));
    return {
        ...preferences,
        fontSize: Number.isFinite(Number(preferences.fontSize)) ? Math.min(32, Math.max(12, Number(preferences.fontSize))) : 20,
        providerMode: preferences.providerMode === 'local' ? 'local' : 'byok',
        answerProvider:
            ids.has(preferences.answerProvider) || ['auto', 'default'].includes(preferences.answerProvider) ? preferences.answerProvider : 'default',
        providerModels: filter(preferences.providerModels),
        enabledProviderModels: filter(preferences.enabledProviderModels),
    };
}
...
function getCredentials() {
    // Provider keys in .env are intentionally authoritative: this keeps a
    // running app in sync with edits made outside the Settings view and avoids
    // touching encrypted storage during ordinary provider-status reads.
    ...
    if (!raw[CREDENTIALS_ENCRYPTION_MARKER]) {
        ...
        // Only migrate legacy plaintext when the OS credential store is ready.
        // A read must not fail merely because encryption is unavailable.
        if (isSecureStorageAvailable()) setCredentials(credentials);
        return credentials;
    }
...
function getEnvCredentials() {
    try {
        const { PROVIDER_KEYS, readProviderEnv } = require('./utils/providerEnv');
        const env = readProviderEnv();
        if (!env) return null;
        const credentials = { ...DEFAULT_CREDENTIALS };
        for (const { envKey, credential } of Object.values(PROVIDER_KEYS)) {
            credentials[credential] = require('./utils/apiKeys').providerApiKeys(env, envKey).join(',');
        }
        return credentials;
    } catch {
        return null;
...
function getPreferences() {
    ...
    return normalizeProviderPreferences({ ...DEFAULT_PREFERENCES, ...saved, selectedLanguage: normalizeLanguageCode(saved.selectedLanguage) });
}
...
function incrementCharUsage(provider, model, charCount) {
    ...
    if (!todayEntry[provider]) todayEntry[provider] = {};
    if (!todayEntry[provider][model]) todayEntry[provider][model] = { chars: 0 };
    todayEntry[provider][model].chars += Math.max(0, Number(charCount) || 0);
    setLimits(limits);
...
function recordTokenUsage(provider, model, usage) {
    ...
    getTodayLimits();
    const limits = getLimits();
    const entry = limits.data.find(item => item.date === getTodayDateString());
    entry.tokenUsage ||= {};
    const key = `${provider}/${model}`;
    const counter = (entry.tokenUsage[key] ||= { input: 0, output: 0, requests: 0 });
    counter.input += Math.max(0, Number(usage.prompt_tokens) || 0);
    counter.output += Math.max(0, Number(usage.completion_tokens) || 0);
    counter.requests++;
    setLimits(limits);
}
