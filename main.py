...
def wait_for_readiness(process=None) -> bool:
...
def load_env(path: Path) -> None:
    for key, previous in _loaded_env.items():
...
def provider_keys(env_key: str) -> list[str]:
    values = [value for key, value in os.environ.items()
              if key == env_key or re.fullmatch(re.escape(env_key) + r"_\d+", key)]
    return list(dict.fromkeys(part.strip() for value in values for part in value.split(",")
                             if part.strip() and not part.strip().startswith("#")))


def configured_providers() -> list[str]:
    return [name for name, env_key in PROVIDERS.items() if provider_keys(env_key)]
...
def show_provider_status() -> int:
    ...
    for index, name in enumerate(PROVIDERS, start=1):
        print(f"  {index}. {name:<10} {str(len(provider_keys(PROVIDERS[name]))) + ' key(s)' if name in available else 'missing'}")
    return 0
...
def stop_running_project() -> None:
    if os.name == "nt":
        ...
        script = (
            ...
            "$_.ExecutablePath.StartsWith(($root + '\\node_modules\\electron\\'), [StringComparison]::OrdinalIgnoreCase) }; "
...
def repository_matches_expected_remote(remote_url: str) -> bool:
    ...
    return normalized in {expected, "git@github.com:rishuburnwal/shadow-ai", "ssh://git@github.com/rishuburnwal/shadow-ai"}
...
def update_project(*, restart: bool = True, quiet_current: bool = False, skip_if_dirty: bool = False) -> int:
    ...
    try:
        ...
        # Never stop a running application for an update that cannot fast-forward.
        if subprocess.run([git, "merge-base", "--is-ancestor", remote_hash, local_hash], cwd=ROOT, check=False).returncode == 0:
            success("Working copy already contains the remote update")
            return 0
        if subprocess.run([git, "merge-base", "--is-ancestor", local_hash, remote_hash], cwd=ROOT, check=False).returncode:
            raise RuntimeError("Local and remote commits diverged. Resolve the branch before updating.")
...
def setup_project(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if not node:
            raise RuntimeError("Node.js was not found on PATH. Install Node.js 20 or newer, then run --setup again.")
...
def launch(args: argparse.Namespace) -> int:
    ...
    if args.providers:
        for name in PROVIDERS:
            print(f"{name}: {str(len(provider_keys(PROVIDERS[name]))) + ' key(s)' if name in available else 'missing'}")
        return 0
    ...
    ready = wait_for_readiness(child)
    if ready:
...
def main() -> int:
    try:
        ...
        return launch(build_parser().parse_args())
    except (RuntimeError, OSError) as error:
        print(f"ERROR: {error}", file=sys.stderr)
        return 2
