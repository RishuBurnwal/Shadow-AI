...
test('config migration — preferences migration adds missing defaults', () => {
    ...
    assert.equal(typeof prefs.jobDescription, 'string');
    assert.equal(typeof prefs.companyContext, 'string');
...
test('starter skills are installed once and deleted presets stay deleted', () => {
    writeRawConfig(2);
    writeRawPreferences({ promptSkills: [], starterSkillsVersion: 1 });
    storage.initializeStorage();
    assert.deepEqual(storage.getPreferences().promptSkills, []);
});
