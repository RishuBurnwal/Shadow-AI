...
test('persistent header owns the shared background transparency and passthrough controls', () => {
    ...
    assert.match(appSource, /\.top-drag-bar[\s\S]*?opacity:\s*1/);
    assert.match(appSource, /\.top-drag-bar[\s\S]*?pointer-events:\s*auto/);
...
test('window controls and global shortcuts toggle minimize and maximize states', () => {
    ...
    assert.match(windowSource, /toggleMinimize:\s*isMac \? 'Cmd\+Shift\+M' : 'Ctrl\+Shift\+M'/);
    assert.match(windowSource, /toggleMaximize:\s*isMac \? 'Cmd\+Shift\+X' : 'Ctrl\+Shift\+X'/);
    assert.match(windowSource, /!mainWindow\.isVisible\(\) \|\| mainWindow\.isMinimized\(\)/);
    assert.match(windowSource, /mainWindow\.restore\(\)/);
    assert.match(windowSource, /mainWindow\.unmaximize\(\)/);
    assert.match(appSource, /@click=\$\{\(\) => this\._handleMaximize\(\)\}/);
    assert.match(settingsSource, /Minimize \/ Restore/);
    assert.match(settingsSource, /Maximize \/ Restore/);
});
...
test('every launch opens directly on the job context onboarding page', () => {
    ...
    assert.doesNotMatch(onboardingSource, /Resume, job description, notes/);
...
test('custom skills are persisted prompt instructions with complete CRUD UI', () => {
    ...
    assert.match(viewSource, /How to use skills/);
    assert.match(viewSource, /Combining skills is\s+supported/);
    assert.match(indexSource, /skills:create/);
    assert.match(indexSource, /skills:update/);
    assert.match(indexSource, /skills:delete/);
});

test('resume has one PDF-or-Markdown editor and structured context fields', () => {
    const viewSource = fs.readFileSync(path.join(root, 'src/components/views/AICustomizeView.js'), 'utf8');
    const promptSource = fs.readFileSync(path.join(root, 'src/utils/prompts.js'), 'utf8');
    const rendererSource = fs.readFileSync(path.join(root, 'src/utils/renderer.js'), 'utf8');

    assert.equal((viewSource.match(/Upload PDF/g) || []).length, 1);
    assert.equal((viewSource.match(/paste resume as Markdown/g) || []).length, 1);
    assert.match(viewSource, /Target Role/);
    assert.match(viewSource, /Job Description/);
    assert.match(viewSource, /Company \/ Industry Context/);
    assert.match(viewSource, /Additional Instructions/);
    assert.match(promptSource, /Resume:\\n/);
    assert.match(rendererSource, /function buildUserContext/);
});
